...
def get_outputs(multiplier, img, model, preprocess):
    ...
    predicted_outputs, _ = model(batch_var)#网络前向运行得到结果
    ...
    for m in range(len(multiplier)):
        ...
        heatmap = cv2.resize(heatmap, None, fx=8, fy=8,
                             interpolation=cv2.INTER_CUBIC)#将46*62的输出变为368*496的heatmap图
...
def run_eval(image_dir, anno_dir, vis_dir, image_list_txt, model, preprocess):
    ...
    for i in range(len(img_ids)):
        ...
        oriImg = cv2.imread(os.path.join(image_dir, 'val2014-select/' + img_paths[i]))
        ...
        #cv2.imshow('result_img', to_plot)
        #cv2.waitKey(1)
        # subset indicated how many peoples foun in this image.
        append_result(img_ids[i], subset, candidate, outputs)

    #with open('/home/data/wangchenliang/pytorch-paf-model/pretrained-MobileNet-okhm-kd/results.json', 'w') as f:
    #with open('/home/data/wangchenliang/pytorch-paf-model/pretrained-MobileNet-only-okhm/results.json', 'w') as f:
    #with open('/home/data/wangchenliang/pytorch-paf-model/pretrained-MobileNet-only-okhm-start-100/results.json', 'w') as f:
    # all start
    with open('/home/data/wangchenliang/pytorch-paf-model/pretrained-MobileNet-all-start/results.json',  'w') as f:
        f.write(json.dump(outputs, f))
    ...
    # return eval_coco(outputs=outputs, dataDir=anno_dir, imgIds=img_ids) #2019.4.22  42行
