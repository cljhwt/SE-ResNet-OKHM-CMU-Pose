...
# import encoding
from network.rtpose_vgg import get_model, use_vgg, use_SEResNet, get_mobileNet_model, use_MobileNet, Net_other, \
    get_model_res
...
parser.add_argument('--init_model_param',
                    default='/home/data/wangchenliang/pytorch-paf-model/pretrained-MobileNet-only-okhm-start-100/init_model_param.pth',
                    type=str)
...
parser.add_argument('--logdir',
                    default='/home/data/wangchenliang/pytorch-paf-model/pretrained-MobileNet-only-okhm-start-100/tensorboy',
...
parser.add_argument('--json_path', default='/home/data/wangchenliang/COCO2014/data/coco/COCO.json', type=str,
                    metavar='PATH',
                    help='path to where coco images stored')  # json标注文件存储位置

# parser.add_argument('--model_path', default='./network/weight/', type=str, metavar='DIR',
#                     help='path to where the model saved') #生成的网络模型保存位置
parser.add_argument('--model_path', default='/home/data/wangchenliang/pytorch-paf-model/ori_model/', type=str,
                    metavar='DIR',
...
parser.add_argument('--gpu_ids', dest='gpu_ids', help='which gpu to use', nargs="+",
                    default=[2, ], type=int)  # nargs="+"表示至少一个
...
train_data = get_loader(args.json_path, args.data_dir,
                        ...
                        shuffle=True, training=True, num_workers=6)
...
valid_data = get_loader(args.json_path, args.data_dir, args.mask_dir, 368,
                        8, preprocess='vgg', training=False,
                        batch_size=16, params_transform=params_transform, shuffle=False, num_workers=4)
...
# 最终模型保存位置
model_save_filename = '/home/data/wangchenliang/pytorch-paf-model/pretrained-MobileNet-only-okhm-start-100/best_pose-pretrained-MobileNet.pth'
# 前五个epoch产生的模型保存位置
model_save_filename1 = '/home/data/wangchenliang/pytorch-paf-model/pretrained-MobileNet-only-okhm-start-100/best_pose-pretrained-MobileNet-epoch5.pth'
# 模型参数保存位置
model_load_all_para_save_filename = '/home/data/wangchenliang/pytorch-paf-model/pretrained-MobileNet-only-okhm-start-100/model-save-para/model_all_para.pth'
#加载参数模型的位置
model_all_para_save_filename = '/home/data/wangchenliang/pytorch-paf-model/pretrained-MobileNet-only-okhm-start-100/model-save-para/model_all_para.pth'
...
if load_train_model_para == True:
    model_CKPT = torch.load(model_load_all_para_save_filename)
...
for epoch in range(epoch_para, args.epochs):
    ...
    # 保存网络中间产生的所有参数
    torch.save({'epoch': epoch + 1, 'state_dict': model.state_dict(), 'best_val_loss': best_val_loss,
                'optimizer': optimizer.state_dict(), 'lr_scheduler': lr_scheduler.state_dict()},
               model_all_para_save_filename)
...
writer.export_scalars_to_json(
    os.path.join('/home/data/wangchenliang/pytorch-paf-model/pretrained-MobileNet-only-okhm-start-100', "all_scalars.json"))
writer.close()
