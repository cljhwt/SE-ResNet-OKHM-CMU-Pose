...
class Cocokeypoints(Dataset):
    ...
    def get_ground_truth(self, meta, mask_miss):
        ...
        for i in range(18):
            ...
            for j in range(nop): #为每个人防止高斯概率图
...
#        print self.root + 'mask2014/val2014_mask_miss_' + img_idx + 'png'
        ...
        self.params_transform['sigma'] = 7
        _, heatmaps_0, _, _ = self.get_ground_truth(
            meta_data, mask_miss)  # sigma = 15

        self.params_transform['sigma'] = 7
        _, heatmaps_1, _, _ = self.get_ground_truth(
            meta_data, mask_miss)  # sigma = 11

        self.params_transform['sigma'] = 7
        _, heatmaps_2, _, _ = self.get_ground_truth(
            meta_data, mask_miss)  # sigma = 3
        ...
        heatmaps_0 = torch.from_numpy(
            heatmaps_0.transpose((2, 0, 1)).astype(np.float32))

        heatmaps_1 = torch.from_numpy(
            heatmaps_1.transpose((2, 0, 1)).astype(np.float32))

        heatmaps_2 = torch.from_numpy(
            heatmaps_2.transpose((2, 0, 1)).astype(np.float32))

        heatmaps_total = []
        heatmaps_total.append(heatmaps_0)
        heatmaps_total.append(heatmaps_1)
        heatmaps_total.append(heatmaps)
        heatmaps_total.append(heatmaps_2)
        ...
        return img, heatmaps_total, heat_mask, pafs, paf_mask
